...
parser.add_argument('-e', '--embeddings', type=str,
                     default='/Users/cecilie/Documents/Code/identification/Features/feret_arcface512_float/feret_arcface512_npy')

parser.add_argument('-cl', '--cluster_centers', type=str,
                     default='/Users/cecilie/Documents/Code/identification/Features/feret_arcface512_float/workload_k_means/feret_cluster_centers')

parser.add_argument('-o', '--output', type=str,
                     default='/Users/cecilie/Documents/Code/identification/Features/feret_arcface512_float/workload_k_means')
...
with open(fpath_txt, 'w') as f: 
    ...
    for i in range(args.k_fold):
        ...
        subjects_and_hashes_file_txt = os.path.join(args.output, "feret_results_subject_id_to_cluster.txt")

        with open(subjects_and_hashes_file_txt, 'w') as file: 

            for i in range(args.k_fold):

                file.write('Link subject id to cluster for iteration {} \n'.format(i)) 
        ...
        cluster_id_to_subjects_file_txt = os.path.join(args.output, "feret_results_cluster_id_to_subjects.txt")
        ...
        for i in range(len(kmeans_centers)): 
            ...
            centers_file_txt = os.path.join(args.cluster_centers, "feret_cluster_{}.npy".format(i))
        ...
        kmeans_centers_file_txt = os.path.join(args.output, "feret_results_kmeans_centers")
        ...
        clustes_file_txt = os.path.join(args.output, "feret_clusters.txt")
        ...
        # Compute average comparison, candidate list, ...
        ...
        mated_scores_file_txt_v2 = os.path.join(args.output, "feret_mated_scores_v2")

        non_mated_scores_file_txt_v2 = os.path.join(args.output, "feret_non_mated_scores_v2")

        m,n= model_identification.func_score_vales_v3(embeddings_path, mated_scores_file_txt_v2, non_mated_scores_file_txt_v2, samples_f, samples_l)
        ...
        f.write('Total non-found: {}, accuracy:{}\n'.format(false_negative, (len(search_l) - false_negative)/len(search_l)))
...
probe_feature_measure_file = os.path.join(args.output, "feret_results_measure_probe_stable_hash")
with open(probe_feature_measure_file, 'w') as file: 
    ...
    file.write("Running {} executions and finding the median time".format(number_of_executions))

    for i in range (number_of_executions): 

        probe_number = random.randint(0, 1413)

        probe = embeddings_path[probe_number]

        file.write("\n\nProbe feature: {} {}\n".format(probe.stem.split('_')[0], probe.stem.split('_')[1]))
        ...
        for i in range(number_of_executions):
            ...
            probe_cluster = sorted_probe_comparison[0][0]
